<template>
  <div>
    ...
    <el-row type="flex" justify="center" style="margin-top: 20px">
      <el-input :placeholder="placeholder" v-model.trim="searchKey" class="search" @keyup.enter.native="handleSearch()">
        <i slot="prefix" class="el-icon-search center"></i>
        <el-link slot="suffix" type="primary" class="text" @click="handleSearch()">搜索</el-link>
      </el-input>
    </el-row>
    <div class="container main">
      <div class="left">
        ...
        <el-radio-group v-if="!type && mainType === '医院信息'" v-model="grade" @change="handleSearch($event)">
        ...
        <span class="rank" v-if="!type && mainType === '疫苗信息'">类别：</span>
        <el-radio-group v-if="!type && mainType === '疫苗信息'" v-model="vType" @change="handleSearch($event)">
          <el-radio-button label="全部"></el-radio-button>
          <el-radio-button label="成人"></el-radio-button>
          <el-radio-button label="儿童"></el-radio-button>
        </el-radio-group>
        <span class="title title-block">{{ hospitalText }}</span>
        <div class="list" id="hospitalList" v-if="mainType == '医院信息'" v-show="!type">
          <Card v-for="item of hospitalList" :key="item.id" :item="item" :type="mainType"></Card>
          <div></div>
          <div></div>
          <div></div>
        </div>
    ...
    methods: {
      handleSearch(grade) {
        if (this.searchKey !== this.prevKey || grade) {
          ...
          if (this.gradeValue && this.mainType === '医院信息') param.grade = this.gradeValue;
          if (this.mainType === '医院信息') {
            searchHospital(param).then(res => {
              this.type = res.data.data.type;
              this.prevKey = this.searchKey;
              this.hospitalText = res.data.data.type ? '组团信息' : this.searchKey ? '医院搜索结果' : '全部医院';
            ...
            searchVaccine(param).then(res => {
              this.type = res.data.data.type;
              this.prevKey = this.searchKey;
              this.hospitalText = res.data.data.type ? '组团信息' : this.searchKey ? '疫苗搜索结果' : '全部疫苗';
              this.hospitalList = res.data.data.data;
              this.total = res.data.data.total;
            });
          }
