    ...
    <template v-if="data.length">
      <el-descriptions v-for="item of data" :key="item.hospitalId">
        <template slot="title">
          <span
            class="hospitalName"
            @click="$router.push('/hospital/' + item.hospitalId)"
        ...
        <template slot="extra">
          <el-button
            ...
            type="primary"
            @click="
              handleOrder('个人预约', item.hospitalId, item.vaccineSpecId)
            "
            >个人预约</el-button
          >
          <el-button
            ...
            type="primary"
            @click="
              handleOrder('家庭预约', item.hospitalId, item.vaccineSpecId)
            "
            >家庭预约</el-button
          >
          <el-button
            ...
            type="primary"
            @click="
              handleOrder('组团预约', item.hospitalId, item.vaccineSpecId)
            "
            v-if="isGroup"
            >组团预约</el-button
          >
        ...
        <el-descriptions-item label="联系电话2" v-if="item.mobileBk">{{
          item.mobileBk
        }}</el-descriptions-item>
        <el-descriptions-item label="工作时间">
          {{ item.workTime }}-{{ item.closeTime }}
        </el-descriptions-item>
...
export default {
  ...
  created() {
    let isGroup, name, spec;
    try {
      ({ isGroup, name, spec } = qs.parse(
        decodeURIComponent(window.atob(this.$route.query.params))
      ));
    } catch {
      //
    } finally {
