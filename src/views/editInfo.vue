<template>
  <div class="container">
    ...
    <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" class="form" label-width="80px">
      ...
      <el-form-item class="submit-item">
        <button class="modify-pwd" @click="includePwd = !includePwd">
          <i :class="includePwd ? 'el-icon-remove-outline' : 'el-icon-circle-plus-outline'"></i
          >{{ includePwd ? '取消修改密码' : '修改密码' }}
        </button>
      </el-form-item>
      <el-form-item><el-button type="primary" @click="handleSubmit">修改</el-button> </el-form-item>
    </el-form>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      handleAvatarSuccess(res, file) {
        let newFace = res.data;
        this.imageUrl = URL.createObjectURL(file.raw);
        this.$store.commit('modify', { userInfo: { ...this.$store.state.userInfo, face: newFace } });
        local.set('info', JSON.stringify({ ...local.get('info'), face: newFace }));
      },
